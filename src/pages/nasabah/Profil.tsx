  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(userRef, {
        ...
        phone,
        address,
        password, // Still stored in doc as per original app
    ...
    <div className="max-w-2xl mx-auto">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="pt-16 pb-6 px-8 border-b border-gray-100">
          ...
          <p className="text-emerald-600 font-medium">{userProfile.id} • Nasabah Aktif</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-4 border-t border-gray-100">
              ...
              <div className="relative">
                <input
                  ...
                  className="block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm pr-10"
                />
                <button
                  type="button"
            ...
            <div className="pt-4">
              <button
                type="submit"
                disabled={loading}
                className={`flex items-center justify-center px-6 py-2.5 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                {loading ? (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mt-6 mb-8">
        <div className="p-6 border-b border-gray-50 flex items-center space-x-2">
          ...
          <h3 className="font-bold text-gray-900">Keamanan Perangkat</h3>
        </div>
        <div className="p-6">
          <p className="text-sm text-gray-500 mb-6">
            Daftar perangkat yang baru saja mengakses akun EcoBank Anda. 
            Hapus perangkat jika Anda tidak mengenalinya.
          </p>
          ...
          <div className="space-y-4">
            {sessions.length === 0 ? (
              ...
              sessions.map((session) => (
                <div 
                  key={session.id} 
                  className={`flex items-center justify-between p-4 rounded-xl border ${session.isCurrent ? 'bg-emerald-50 border-emerald-100' : 'bg-gray-50 border-gray-100'}`}
                >
                  <div className="flex items-center space-x-4">
                    <div className={`p-3 rounded-xl ${session.isCurrent ? 'bg-emerald-600 text-white' : 'bg-white text-gray-400 shadow-sm'}`}>
                      <Smartphone size={24} />
                    </div>
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        {session.isCurrent && (
                          <span className="bg-emerald-200 text-emerald-800 text-[10px] font-black px-1.5 py-0.5 rounded uppercase">
                            Sesi Ini
                          </span>
                        )}
                      </div>
      ...
      <PasswordVerificationModal 
        ...
        description="Konfirmasi penghapusan sesi ini? Akun pada perangkat tersebut akan secara otomatis keluar."
      />
    </div>
  );
}
